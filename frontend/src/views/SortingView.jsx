...
export default function SortingView() {
  ...
  // 1. ESTADO DE TELEMETRÍA
  const [telemetria, setTelemetria] = useState({
    cajas_pequenas: 0,
    cajas_grandes: 0,
    correa_principal: false,
  ...
  const operatingShield = useRef(false); // 

  // Estado visual mapeado directamente desde la base de datos
  ...
  const [tiempoActivo, setTiempoActivo] = useState(() => {
    return parseInt(localStorage.getItem('scada_p1_time')) || 0;
  }); 

  useEffect(() => {
    localStorage.setItem('scada_p1_time', tiempoActivo);
  }, [tiempoActivo]);

  const fetchCommandsLog = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/factory/historial-comandos');
      if (res.ok) {
        const data = await res.json();
        setHistorialComandos(data);
      }
    } catch (error) {
      console.error("Error leyendo log de comandos P1:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchTelemetria = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/factory/telemetria');
        if (res.ok) {
          const data = await res.json();
          ...
          if (!operatingShield.current) {
            ...
            // Sincronizar Estado de Marcha directo del PLC
          ...
          // Gráfico en tiempo real (Deslizamiento SCADA de 30 segundos)
          const timestamp = new Date().toLocaleTimeString('es-CL', { hour12: false });
          setHistorico(prev => {
            const nuevoDato = {
              tiempo: timestamp,
              Pequeñas: data.cajas_pequenas || 0,
              Grandes: data.cajas_grandes || 0
            };
            return [...prev, nuevoDato].slice(-30);
          });
        }
      } catch (error) {
        console.error("Error al obtener telemetría P1", error);
      }
    };

    // Llamada síncrona al montar
    fetchTelemetria(); 
  ...
  // 4. Envío de comandos: Actualización optimista de UI + Escudo temporal
  const handleControl = async (comando) => {
    setIsOperating(true);
    operatingShield.current = true; // Levantamos el escudo

    // Optimismo visual inmediato para el operador
    ...
    try {
      const res = await fetch('http://localhost:5000/api/factory/control', {
        method: 'POST',
  ...
  // Cálculos de Rendimiento (KPIs Dinámicos)
  const totalCajas = telemetria.cajas_pequenas + telemetria.cajas_grandes;
  const horasTrabajadas = tiempoActivo > 0 ? (tiempoActivo / 3600) : 0;
  const pequeñasPorHora = horasTrabajadas > 0 ? (telemetria.cajas_pequenas / horasTrabajadas).toFixed(0) : 0;
  const grandesPorHora = horasTrabajadas > 0 ? (telemetria.cajas_grandes / horasTrabajadas).toFixed(0) : 0;
    ...
    <div style={styles.dashboardContainer}>
      <header style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <h2 style={styles.headerTitle}>Clasificadora por Altura</h2>
        </div>
      ...
      <main style={styles.mainGrid}>
        ...
        <section style={{...styles.card, gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '15px'}}>
          ...
          <div style={styles.controlGroup}>
            <button 
              disabled={estadoProceso === 'EN MARCHA' || isOperating} 
              onClick={() => handleControl('START')} 
              style={{...styles.btnStart, opacity: (estadoProceso === 'EN MARCHA' || isOperating) ? 0.5 : 1}}
            >
        ...
        {/* RENDIMIENTO GLOBAL (KPIs) */}
        <section style={{...styles.card, gridColumn: 'span 1'}}>
          <h3 style={styles.cardTitle}>Producción Acumulada</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <div style={styles.kpiValueRow}>
              <Layers size={24} color="#0f172a" />
              <div>
                <span style={styles.kpiLabel}>Total Clasificadas</span>
                <span style={styles.kpiNumber}>{totalCajas}</span>
              </div>
            </div>
            <div style={styles.kpiValueRow}>
              <Box size={24} color="#3b82f6" />
              <div>
                <span style={styles.kpiLabel}>Cajas Pequeñas</span>
                <span style={styles.kpiNumber}>{telemetria.cajas_pequenas}</span>
              </div>
            </div>
            <div style={styles.kpiValueRow}>
              <Box size={24} color="#f59e0b" />
              <div>
                <span style={styles.kpiLabel}>Cajas Grandes</span>
                <span style={styles.kpiNumber}>{telemetria.cajas_grandes}</span>
              </div>
            </div>
          </div>
        </section>

        {/* ESTADÍSTICAS DE FLUJO Y TIEMPO */}
        <section style={{...styles.card, gridColumn: 'span 2'}}>
          <h3 style={styles.cardTitle}>Métricas de Eficiencia</h3>
          <div style={styles.kpiGrid}>
            <div style={styles.kpiBox}>
              <div style={styles.kpiValueRow}>
                <Clock size={24} color="#64748b" />
                <div>
                  <span style={styles.kpiLabel}>Tiempo Operativo</span>
                  <span style={styles.kpiNumber}>{formatTime(tiempoActivo)}</span>
                </div>
              </div>
            </div>
            <div style={styles.kpiBox}>
              <div style={styles.kpiValueRow}>
                ...
                <div>
                  <span style={styles.kpiLabel}>Tasa Pequeñas (uds/h)</span>
                  <span style={styles.kpiNumber}>{pequeñasPorHora}</span>
                </div>
              </div>
            </div>
            <div style={styles.kpiBox}>
              <div style={styles.kpiValueRow}>
                ...
                <div>
                  <span style={styles.kpiLabel}>Tasa Grandes (uds/h)</span>
                  <span style={styles.kpiNumber}>{grandesPorHora}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* MONITOREO DE ACTUADORES (CORREAS) */}
        <section style={{...styles.card, gridColumn: '1 / -1'}}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '15px', marginTop: '10px' }}>
            <div style={styles.actuatorBox(isStateActive(telemetria.correa_principal))}>
              <span style={styles.actuatorLabel}>Correa Principal</span>
              <strong style={{ fontSize: '15px' }}>{isStateActive(telemetria.correa_principal) ? 'TRABAJANDO' : 'DETENIDA'}</strong>
            </div>
            <div style={styles.actuatorBox(isStateActive(telemetria.correa_izquierda))}>
              <span style={styles.actuatorLabel}>Correa Izquierda (Pequeñas)</span>
            ...
            <div style={styles.actuatorBox(isStateActive(telemetria.correa_derecha))}>
              <span style={styles.actuatorLabel}>Correa Derecha (Grandes)</span>
        ...
        {/* GRÁFICO HISTÓRICO */}
        <section style={{...styles.card, gridColumn: '1 / -1'}}>
          <h3 style={styles.cardTitle}>Curva de Clasificación</h3>
          <div style={{ width: '100%', height: '250px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historico} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorPeq" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="stepAfter" dataKey="Pequeñas" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorPeq)" isAnimationActive={false} />
                <Area type="stepAfter" dataKey="Grandes" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorGra)" isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </section>
                ...
                {historialComandos.length === 0 ? (
                  ...
                  historialComandos.map((item, index) => (
                    <tr key={index} style={styles.tableBodyRow(index)}>
                      ...
                      <td style={{ ...styles.tableBodyCell, color: '#64748b', fontSize: '13px' }}>
                        {item.hora}
                      </td>
                    </tr>
                  ))
